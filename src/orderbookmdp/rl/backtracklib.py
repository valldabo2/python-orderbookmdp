...
class Solver:
    ...
    def solve(self, num_answers=1, max_time=0, threading=False):
        ...
        time_start = time()
        if not threading:
            limit = self._recursive_solve(parcial, answers, num_answers if num_answers != 0 else float('inf'), max_time,
                                          time_start)
        else:
            limit = self._threaded_recursive_solve(parcial, answers, num_answers if num_answers != 0 else float('inf'),
                                                   max_time, time_start)
    ...
    def _threaded_recursive_solve(self, parcial, answers, num_answers, max_time, time_start):
        for elem in self.CalcularPosibles(parcial):
            thread = Thread(target=self._recursive_solve,
                            args=(parcial, answers, num_answers, max_time, time_start, True))
            thread.start()
